...
estadd local fe1 "No"
esttab using "${tables}/table_ex5.tex", replace label title(Determinants) noobs keep(mean_ambition `controls') order(mean_ambition `controls') s(N r2_p fe1, label("N" "Psuedo R-squared" "Industry FE") fmt(0 %9.3f)) star(* .10 ** .05 *** .01) nocon b(3) unstack eqlabels("Achieved vs. Failed" "Achieved vs. Disappeared") collab(none) mlabels(none) drop(1:)
