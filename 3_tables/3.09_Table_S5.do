...
estadd local fe1 "Yes"
estadd local fe2 "Yes"
esttab using "${tables}/table_s5.tex", replace label title(Consequences of Failing Targets) noobs nobaselevels order(4.i_type#1.post 2.i_type#1.post 3.i_type#1.post `controls') keep(4.i_type#1.post 2.i_type#1.post 3.i_type#1.post `controls') interaction(" X ") s(N r2_a fe1 fe2, label("N" "Adj. R-squared" "Firm FE" "Year FE") fmt(0 %9.3f)) star(* .10 ** .05 *** .01) nocon b(3)
